...
public class Solution2C1Activity extends AppCompatActivity {
    ...
    private List<Call> mCallCatList = new ArrayList<>();
    ...
    public void requestData(View view) {
        ...
        Call<List<Cat>> CatCall = NetworkUtils.getResponseWithRetrofitAsync1();
        mCallCatList.add(CatCall);
        CatCall.enqueue(new Callback<List<Cat>>() {
            @Override
            public void onResponse(Call<List<Cat>> call, Response<List<Cat>> response) {
                    loadPics(response.body());
                    restoreBtn();
                    mCallCatList.remove(call);
            }
    ...
    private void loadPics(List<Cat> cats) {  //加载图片
        mCats = cats;
        mRv.getAdapter().notifyDataSetChanged();
    }

    private void restoreBtn() {   //刷新按钮？
